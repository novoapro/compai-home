...
final class LoggingServiceTests: XCTestCase {
    ...
    // MARK: - Filter Logs by Automation ID

    func testGetLogs_forAutomationId() async {
        let automationId = UUID()
        let otherAutomationId = UUID()

        // Create automation execution logs
        let automationLog1 = StateChangeLog(
            id: UUID(),
            timestamp: Date(),
            category: .automationExecution,
        ...
        let automationLog2 = StateChangeLog(
            id: UUID(),
            timestamp: Date(),
            category: .automationExecution,
            payload: .automationExecution(
        ...
        await loggingService.logEntry(automationLog1)
        await loggingService.logEntry(automationLog2)
        await loggingService.logEntry(stateChangeLog)

        let filteredLogs = await loggingService.getLogs(forAutomationId: automationId)
        XCTAssertEqual(filteredLogs.count, 1)
        XCTAssertEqual(filteredLogs[0].automationExecution?.automationId, automationId)
    }

    func testGetLogs_forAutomationId_emptyResult() async {
        let automationId = UUID()
        let otherAutomationId = UUID()

        let automationLog = StateChangeLog(
            id: UUID(),
            timestamp: Date(),
            category: .automationExecution,
            payload: .automationExecution(
        ...
        await loggingService.logEntry(automationLog)

        let filteredLogs = await loggingService.getLogs(forAutomationId: automationId)
        XCTAssertEqual(filteredLogs.count, 0)
    }

    // MARK: - Clear Logs by Automation ID

    func testClearLogs_forAutomationId() async {
        let automationId = UUID()
        let otherAutomationId = UUID()
        ...
        await loggingService.clearLogs(forAutomationId: automationId)

        let logsAfter = await loggingService.getLogs()
        XCTAssertEqual(logsAfter.count, 1)
        XCTAssertEqual(logsAfter[0].automationExecution?.automationId, otherAutomationId)
    }
...
class MockStorageService: StorageServiceProtocol {
    ...
    func readAutomationLoggingEnabled() -> Bool { true }
