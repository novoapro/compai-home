...
import type { StateChangeLog, DeviceStateLog, AutomationLog } from '@/types/state-change-log';
import { LogCategory } from '@/types/state-change-log';
import type { AutomationExecutionLog, ExecutionStatus } from '@/types/automation-log';

/** Create a minimal valid AutomationExecutionLog. */
function makeAutomationExec(status: ExecutionStatus = 'success'): AutomationExecutionLog {
  return {
    id: 'exec-1',
    automationId: 'wf-1',
...
function makeLog(overrides: { id: string; timestamp?: string; automationExecution?: AutomationExecutionLog }): StateChangeLog {
  if (overrides.automationExecution) {
    const isError = overrides.automationExecution.status === 'failure';
    return {
      id: overrides.id,
...
describe('usePolling', () => {
  ...
  beforeEach(() => {
    ...
    mockApi = {
      getLogs: vi.fn().mockResolvedValue({
        logs: [makeLog({ id: '1', automationExecution: makeAutomationExec('success') })],
        total: 1,
      }),
    };
  ...
  it('loadMore appends logs without replacing existing ones', async () => {
    mockApi.getLogs.mockResolvedValueOnce({
      logs: [
        makeLog({ id: '1', timestamp: '2024-01-01T00:00:00Z', automationExecution: makeAutomationExec('success') }),
      ],
      total: 3,
    });
    ...
    mockApi.getLogs.mockResolvedValueOnce({
      logs: [
        makeLog({ id: '2', timestamp: '2024-01-01T00:01:00Z', automationExecution: makeAutomationExec('running') }),
      ],
      total: 3,
    });
  ...
  it('injectLog adds new log to the beginning', () => {
    ...
    const log = makeLog({ id: 'new-log', automationExecution: makeAutomationExec('success') });
  ...
  it('does not inject duplicate logs', () => {
    ...
    const log = makeLog({ id: 'duplicate-id', automationExecution: makeAutomationExec('success') });
  ...
  it('updateLog modifies existing log in place', () => {
    const log = makeLog({ id: 'log-1', automationExecution: makeAutomationExec('running') });
    ...
    const updatedLog = makeLog({ id: 'log-1', automationExecution: makeAutomationExec('success') });

    act(() => {
      result.current.updateLog(updatedLog);
    });

    const updated = result.current.logs[0] as AutomationLog;
    expect(updated.automationExecution.status).toBe('success');
    expect(result.current.logs).toHaveLength(1);
  });

  it('does not overwrite terminal status with running status', () => {
    const log = makeLog({ id: 'log-1', automationExecution: makeAutomationExec('success') });
    ...
    const staleUpdate = makeLog({ id: 'log-1', automationExecution: makeAutomationExec('running') });

    act(() => {
      result.current.updateLog(staleUpdate);
    });

    const kept = result.current.logs[0] as AutomationLog;
    expect(kept.automationExecution.status).toBe('success');
  });
  ...
  it('tracks latest timestamp for delta polling', async () => {
    mockApi.getLogs.mockResolvedValue({
      logs: [
        makeLog({ id: '1', timestamp: '2024-01-01T00:00:10Z', automationExecution: makeAutomationExec('success') }),
      ],
      total: 1,
    });
  ...
  it('handles error in loadMore', async () => {
    mockApi.getLogs.mockResolvedValueOnce({
      logs: [makeLog({ id: '1', timestamp: '2024-01-01T00:00:00Z', automationExecution: makeAutomationExec('success') })],
      total: 2,
    });
