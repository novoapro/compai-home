...
function getAutomationStatus(log: StateChangeLog | undefined): string | undefined {
  if (!log) return undefined;
  if (log.category === LogCategory.AutomationExecution || log.category === LogCategory.AutomationError) {
    return log.automationExecution.status;
  }
  return undefined;
}
...
function pollingReducer(state: PollingState, action: PollingAction): PollingState {
  switch (action.type) {
    ...
    case 'UPDATE_LOG': {
      ...
      const existingStatus = getAutomationStatus(state.logs[idx]);
      const incomingStatus = getAutomationStatus(action.payload);
