...
import { newConditionLeaf, conditionAutoName } from './automation-editor-utils';
import type { BlockInfo } from './automation-editor-utils';
import './ConditionGroupEditor.css';
import './AutomationEditorPage.css'; // for wfe-add-menu styles
...
export function ConditionGroupEditor({
  ...
  const innerGroup = useMemo((): AutomationConditionDraft => {
  ...
  function isGroup(c: AutomationConditionDraft): boolean {
    return c.type === 'and' || c.type === 'or' || c.type === 'not';
  }

  function childName(c: AutomationConditionDraft): string {
  ...
  function groupMeta(c: AutomationConditionDraft): string {
    let count = 0;
  ...
  function badgeFor(c: AutomationConditionDraft): string {
  ...
  const patchInner = useCallback(
    (changes: Partial<AutomationConditionDraft>) => {
      const inner: AutomationConditionDraft = { ...innerGroup, ...changes };
      if (isNegated) {
  ...
  const setOperator = useCallback(
    (op: 'and' | 'or') => {
      ...
      const updated: AutomationConditionDraft = { ...innerGroup, type: op };
      if (isNegated) {
  ...
  const addGroup = useCallback(() => {
    ...
    const group: AutomationConditionDraft = { _draftId: newUUID(), type: oppositeOp, conditions: [] };
