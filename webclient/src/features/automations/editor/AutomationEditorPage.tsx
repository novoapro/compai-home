...
import './AutomationEditorPage.css';
...
const TRIGGER_ICONS: Record<string, string> = {
  ...
  sunEvent: 'sun-max',
  webhook: 'globe',
  automation: 'arrow-triangle-branch',
};
...
export function AutomationEditorPage() {
  ...
  useSetTopBar(isEditMode ? 'Edit Automation' : 'New Automation');
  ...
  const panelConditionRef = useRef<AutomationConditionDraft | null>(null);
  ...
  const panelStackRef = useRef<{ condition: AutomationConditionDraft; frame: PanelFrame; childIndex: number }[]>([]);
  ...
  // Load existing automation for edit mode (or accept AI-improved draft via navigation state)
  useEffect(() => {
    if (!isEditMode || !automationId) return;
    (async () => {
      setIsLoading(true);
      try {
        const aiDraft = (location.state as Record<string, unknown> | null)?.aiDraft as AutomationDefinition | undefined;
        if (aiDraft) {
  ...
  }, [api, automationId, isEditMode, reset]);
  ...
  const resolveNestedBlocks = useCallback((): AutomationBlockDraft[] => {
  ...
  const handleTriggerClone = useCallback(
    (index: number) => {
      ...
      if (!original) return;
      const clone: AutomationTriggerDraft = {
  ...
  const setNestedBlocks = useCallback(
    (blocks: AutomationBlockDraft[]) => {
      ...
      const rootBlocks: AutomationBlockDraft[] = JSON.parse(JSON.stringify(draft.blocks));
  ...
  const handleBlockChange = useCallback(
    (index: number, updated: AutomationBlockDraft) => {
      if (nestingStack.length === 0) {
    ...
    (blockId: string, info: { field: string; label: string }) => {
      if (info.field === 'condition') {
        ...
        let condDraft = block.condition
          ? (JSON.parse(JSON.stringify(block.condition)) as AutomationConditionDraft)
  ...
  const getPanelRootCondition = useCallback(() => {
    ...
    let result = JSON.parse(JSON.stringify(panelConditionRef.current)) as AutomationConditionDraft;
    for (let i = panelStackRef.current.length - 1; i >= 0; i--) {
      const entry = panelStackRef.current[i]!;
      const parentCopy: AutomationConditionDraft = JSON.parse(JSON.stringify(entry.condition));
  ...
  const navigatePanelBack = useCallback(() => {
    ...
    const parentCopy: AutomationConditionDraft = JSON.parse(JSON.stringify(popped.condition));
    let innerGroup = parentCopy;
    if (parentCopy.type === 'not' && parentCopy.condition && (parentCopy.condition.type === 'and' || parentCopy.condition.type === 'or')) {
      innerGroup = parentCopy.condition;
    }
    ...
    if (blockId) {
      ...
      if (blockIndex >= 0) {
        if (path.length === 1) {
          ...
          const rootCopy: AutomationConditionDraft = JSON.parse(JSON.stringify(
      ...
      if (path.length === 1) {
        ...
        const rootCopy: AutomationConditionDraft = JSON.parse(JSON.stringify(draft.conditions[0] ?? { _draftId: newUUID(), type: 'and', conditions: [] }));
        let current = rootCopy;
        for (let i = 1; i < path.length - 1; i++) {
          let inner = current;
          if (inner.type === 'not' && inner.condition && (inner.condition.type === 'and' || inner.condition.type === 'or')) {
  ...
  const saveAutomation = useCallback(async () => {
    ...
    try {
      ...
      if (isEditMode && automationId) {
        ...
        await api.updateAutomation(automationId, payload as any);
        markSaved();
        navigate(`/automations/${automationId}/definition`, { replace: true });
      } else {
        const created = await api.createAutomation(payload);
        markSaved();
        navigate(`/automations/${created.id}/definition`, { replace: true });
      }
    } catch (err: unknown) {
  ...
  }, [validationErrors, draft, isEditMode, automationId, api, navigate, markSaved]);
  ...
  return (
    <div className="wfe-page" ref={pageRef}>
      ...
      <h1 className="wfe-page-title">{isEditMode ? 'Edit Automation' : 'New Automation'}</h1>
        ...
        <div className="wfe-detail-fields">
          ...
          <div className="wfe-toggle-row">
            <div>
              ...
              <div className="wfe-toggle-hint">Automation will execute when triggered</div>
            </div>
            <button
              type="button"
      ...
      <AddTriggerSheet
        ...
        onAdd={(type) => {
          addTrigger(type as AutomationTriggerDraft['type']);
        ...
        <div className="wfe-footer-inner">
          ...
          <div className="wfe-footer-actions">
            ...
            <button
              ...
              type="button"
            >
              {isSaving ? 'Saving...' : isEditMode ? 'Save Changes' : 'Create Automation'}
            </button>
          </div>
        </div>
      </div>
