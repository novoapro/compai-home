import type { AutomationBlockDraft, AutomationConditionDraft } from './automation-editor-types';
import { newUUID } from './automation-editor-types';

function newConditionDraft(): AutomationConditionDraft {
...
function regenerateConditionIds(condition: AutomationConditionDraft): AutomationConditionDraft {
  const clone: AutomationConditionDraft = { ...condition, _draftId: newUUID() };
  if (clone.conditions) {
...
export function cloneBlockDraft(block: AutomationBlockDraft): AutomationBlockDraft {
  const clone: AutomationBlockDraft = JSON.parse(JSON.stringify(block));
