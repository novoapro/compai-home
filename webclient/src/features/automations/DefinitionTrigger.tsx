...
import type {
  AutomationTriggerDef, DeviceStateTriggerDef,
  ScheduleTriggerDef, SunEventTriggerDef, WebhookTriggerDef,
} from '@/types/automation-definition';
import { TRIGGER_TYPE_ICONS } from '@/types/automation-log';
import { formatTriggerCondition, formatScheduleType, formatRetriggerPolicy } from '@/utils/automation-definition-utils';
...
export function DefinitionTrigger({ trigger, depth = 0 }: DefinitionTriggerProps) {
  ...
  const displayName = useMemo(() => {
    ...
    switch (trigger.type) {
      ...
      case 'automation': return 'Callable Trigger';
      case 'sunEvent': {
        const s = trigger as SunEventTriggerDef;
        return s.event === 'sunrise' ? 'Sunrise' : 'Sunset';
      }
      default: return (trigger as AutomationTriggerDef).type;
    }
  }, [trigger, registry]);
  ...
  const detailText = useMemo(() => {
    switch (trigger.type) {
      ...
      case 'automation': return 'Can be triggered by other automations';
        ...
        <div className="tree-trigger-details" style={{ paddingLeft: depth * 20 + 18, display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          {trigger.type === 'webhook' && (trigger as WebhookTriggerDef).token && (
            <button
              ...
              onClick={() => copyUrl(`${baseUrl}/automations/webhook/${(trigger as WebhookTriggerDef).token}`)}
