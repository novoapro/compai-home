...
import { BlockResultTree } from '@/features/automations/BlockResultTree';
import { ConditionResultTree } from '@/features/automations/ConditionResultTree';
...
import type { ExecutionStatus } from '@/types/automation-log';
import './LogRow.css';
...
export const LogRow = memo(function LogRow({ log, index }: LogRowProps) {
  ...
  const isError = useMemo(() => {
    ...
    return cat === LogCategory.WebhookError ||
      ...
      cat === LogCategory.AutomationError ||
  ...
  const automationStatus = (log.category === LogCategory.AutomationExecution || log.category === LogCategory.AutomationError)
    ? log.automationExecution.status
    : null;
  const automationStatusColor = automationStatus ? (STATUS_COLOR_MAP[automationStatus] || 'var(--text-secondary)') : 'var(--status-active)';
  const automationStatusLabel = automationStatus ? (STATUS_LABEL_MAP[automationStatus] || automationStatus) : '';

  const automationTriggerDescription = useMemo(() => {
    if (log.category !== LogCategory.AutomationExecution && log.category !== LogCategory.AutomationError) return null;
    const e = log.automationExecution;
  ...
  return (
    ...
    >
      <div className="content">
        ...
        {automationStatus ? (
          <div className="status-row">
            <span
              ...
              style={{ color: automationStatusColor, backgroundColor: `${automationStatusColor}22` }}
            >{automationStatusLabel}</span>
          </div>
        ...
        {log.category === LogCategory.AutomationExecution && (
          <>
            {automationTriggerDescription && (
              <div className="api-content">
                <span className="api-text automation-trigger">{automationTriggerDescription}</span>
              </div>
            )}
            {log.automationExecution.errorMessage && (
              <div className="api-result">
                <span className="result-text" style={{ color: automationStatusColor }}>{log.automationExecution.errorMessage}</span>
              </div>
            )}
          </>
        )}

        {log.category === LogCategory.AutomationError && (
          <>
            {automationTriggerDescription && (
              <div className="api-content">
                <span className="api-text automation-trigger">{automationTriggerDescription}</span>
            ...
            {log.automationExecution.errorMessage ? (
              <div className="error-banner-inline">
                <Icon name="exclamation-circle-fill" size={14} />
                <span>{log.automationExecution.errorMessage}</span>
              </div>
            ) : isError ? (
              <div className="error-banner-inline">
                <Icon name="exclamation-circle-fill" size={14} />
      ...
      {expanded && expandable && (
        <div className="detail-inline">
          ...
          ) : (log.category === LogCategory.AutomationExecution || log.category === LogCategory.AutomationError) ? (
            <div className="automation-execution-detail" onClick={(e) => e.stopPropagation()}>
              {log.automationExecution.conditionResults && log.automationExecution.conditionResults.length > 0 && (
                <>
                  <div className="execution-section-label">Conditions</div>
                  <div className="execution-tree-content">
                    {log.automationExecution.conditionResults.map((cond, i) => (
                      <ConditionResultTree key={i} result={cond} depth={0} isFirst={i === 0} isLast={i === log.automationExecution.conditionResults!.length - 1} />
                    ))}
                  </div>
                </>
              )}
              {log.automationExecution.blockResults.length > 0 && (
                <>
                  <div className="execution-section-label">Steps ({log.automationExecution.blockResults.length})</div>
                  <div className="execution-tree-content">
                    {log.automationExecution.blockResults.map((block, i) => (
                      <BlockResultTree key={block.id} result={block} depth={0} isFirst={i === 0} isLast={i === log.automationExecution.blockResults.length - 1} />
                    ))}
                  </div>
                </>
              )}
              ...
              <div className="automation-detail-links">
                <Link
                  className="view-detail-link"
                  to={`/automations/${log.automationExecution.automationId}/${log.automationExecution.id}`}
                ...
                <Link
                  className="view-detail-link"
                  to={`/automations/${log.automationExecution.automationId}/definition`}
                  onClick={(e) => e.stopPropagation()}
                >
                  <Icon name="doc-text" size={14} />
                  View automation definition
                </Link>
              </div>
            </div>
          ) : (
