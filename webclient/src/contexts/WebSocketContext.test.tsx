...
describe('WebSocketContext', () => {
  ...
  it('provides event subscription methods', () => {
    ...
    expect(typeof contextValue.onAutomationLog).toBe('function');
    expect(typeof contextValue.onAutomationsUpdated).toBe('function');
  ...
  it('handles automation_log message type', async () => {
    let contextValue: any;
    ...
    const ws = MockWebSocket.instances[0]!;
    const handler = vi.fn();
    contextValue.onAutomationLog(handler);

    const logData = { id: 'exec-1', status: 'running' };
    act(() => {
      ws.simulateMessage('automation_log', logData);
    });

    expect(handler).toHaveBeenCalledWith({ type: 'new', data: logData });
  });

  it('handles automations_updated message type', async () => {
    let contextValue: any;
    ...
    const ws = MockWebSocket.instances[0]!;
    const handler = vi.fn();
    contextValue.onAutomationsUpdated(handler);

    const automations = [{ id: 'wf-1', name: 'Automation 1' }];
    act(() => {
      ws.simulateMessage('automations_updated', automations);
    });

    expect(handler).toHaveBeenCalledWith(automations);
  });
