...
interface AIImproveDialogProps {
  ...
  originalAutomation: AutomationDefinition;
  onClose: () => void;
  onOpenInEditor: (automation: AutomationDefinition) => void;
}
...
export function AIImproveDialog({
  ...
  const [improved, setImproved] = useState<AutomationDefinition | null>(null);
  ...
  const handleImprove = useCallback(async () => {
    ...
    try {
      ...
      const result = await api.improveAutomation(automationId, trimmed || undefined);
      setImproved(result);
      setPhase('review');
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to improve automation');
      setPhase('error');
    }
  }, [prompt, automationId, api]);
  ...
  const changeSummary = improved
    ? {
        name: improved.name !== originalAutomation.name,
        description: improved.description !== originalAutomation.description,
        triggers: improved.triggers.length !== originalAutomation.triggers.length,
        triggerCount: improved.triggers.length,
        blocks: improved.blocks.length !== originalAutomation.blocks.length,
        blockCount: improved.blocks.length,
        conditions:
          (improved.conditions?.length ?? 0) !== (originalAutomation.conditions?.length ?? 0),
      ...
      <div className="aii-dialog" onClick={(e) => e.stopPropagation()}>
        ...
        {phase === 'loading' && (
          <>
            ...
            <h3 className="aii-title">Improving Automation</h3>
            <div className="aii-spinner" />
            <p className="aii-loading-text">Analyzing and improving your automation...</p>
          </>
        )}
              ...
              <div className="aii-change-items">
                ...
                <span className={`aii-change-chip ${changeSummary.triggers ? 'changed' : ''}`}>
                  ...
                  {changeSummary.triggers && (
                    <span className="aii-change-delta">
                      {' '}(was {originalAutomation.triggers.length})
                    </span>
                  )}
                </span>
                <span className={`aii-change-chip ${changeSummary.blocks ? 'changed' : ''}`}>
                  ...
                  {changeSummary.blocks && (
                    <span className="aii-change-delta">
                      {' '}(was {originalAutomation.blocks.length})
                    </span>
                  )}
                </span>
                {(changeSummary.conditionCount > 0 || (originalAutomation.conditions?.length ?? 0) > 0) && (
                  <span className={`aii-change-chip ${changeSummary.conditions ? 'changed' : ''}`}>
                    ...
                    {changeSummary.conditions && (
                      <span className="aii-change-delta">
                        {' '}(was {originalAutomation.conditions?.length ?? 0})
                      </span>
                    )}
                  </span>
                )}
