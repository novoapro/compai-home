...
import './AutomationExecutionDetailPage.css';
...
export function AutomationExecutionDetailPage() {
  const { automationId, logId } = useParams<{ automationId: string; logId: string }>();
  const navigate = useNavigate();
  const api = useApi();
  const ws = useWebSocket();

  const [log, setLog] = useState<AutomationExecutionLog | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  useSetTopBar('Execution Detail', null, isLoading);

  const loadDetail = useCallback(async () => {
    if (!automationId || !logId) return;
    setIsLoading(true);
    setError(null);
    try {
      const logs = await api.getAutomationLogs(automationId, 100);
          ...
          <div className="wfed-section wfed-header-section animate-fade-in">
            <div className="wfed-header-row">
              ...
              <div className="wfed-header-info">
                <h2 className="wfed-automation-name">{log.automationName}</h2>
