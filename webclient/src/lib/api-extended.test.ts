...
describe('createApiClient - Extended Tests', () => {
  ...
  describe('automation CRUD methods', () => {
    it('createAutomation uses POST method with correct payload', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(JSON.stringify({ id: 'wf-123', name: 'New Automation' })),
      });

      vi.stubGlobal('fetch', mockFetch);

      const client = createApiClient(baseUrl, token);
      const automation = { name: 'Test Automation', description: 'A test' };
      ...
      expect(options.body).toBe(JSON.stringify(automation));
    });

    it('updateAutomation uses PUT method with correct payload', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ok: true,
      ...
      await client.updateAutomation('wf-123', updates);

      const [url, options] = mockFetch.mock.calls[0]!;
      expect(url).toContain('/automations/wf-123');
      expect(options.method).toBe('PUT');
      expect(options.body).toBe(JSON.stringify(updates));
    });

    it('deleteAutomation uses DELETE method', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(''),
      });
      ...
      await client.deleteAutomation('wf-123');
    ...
    it('getAutomation retrieves a single automation', async () => {
      const automation = { id: 'wf-456', name: 'My Automation' };

      const mockFetch = vi.fn().mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(JSON.stringify(automation)),
      });

      vi.stubGlobal('fetch', mockFetch);

      const client = createApiClient(baseUrl, token);
      const result = await client.getAutomation('wf-456');

      expect(result).toEqual(automation);
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/automations/wf-456'),
        expect.any(Object),
      );
    });
    ...
    it('getAutomations returns array of automations', async () => {
      const automations = [
        { id: 'wf-1', name: 'Automation 1' },
        { id: 'wf-2', name: 'Automation 2' },
      ];

      const mockFetch = vi.fn().mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(JSON.stringify(automations)),
      });
  ...
  describe('getLogs with filter parameters', () => {
    it('includes all query parameters in request', async () => {
      ...
      expect(url).toContain('categories=device%2Cautomation');
  ...
  describe('error handling for HTTP status codes', () => {
    ...
    it('throws on 404 not found', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ...
        text: () => Promise.resolve('{"error":"Automation not found"}'),
      });
      ...
      await expect(client.getAutomation('nonexistent')).rejects.toThrow('Automation not found');
    });
  ...
  describe('getAutomationLogs', () => {
    it('includes limit parameter when provided', async () => {
      ...
      const client = createApiClient(baseUrl, token);
      await client.getAutomationLogs('wf-123', 20);

      const [url] = mockFetch.mock.calls[0]!;
      expect(url).toContain('/automations/wf-123/logs');
    ...
    it('omits limit parameter when not provided', async () => {
      ...
      const client = createApiClient(baseUrl, token);
      await client.getAutomationLogs('wf-123');
  ...
  describe('getAutomationRuntime', () => {
    it('retrieves automation runtime configuration', async () => {
      const runtime = {
        sunEvents: {
          sunrise: '06:30',
          sunset: '18:45',
      ...
      const client = createApiClient(baseUrl, token);
      const result = await client.getAutomationRuntime();

      expect(result).toEqual(runtime);
      expect(mockFetch).toHaveBeenCalledWith(
        expect.stringContaining('/automation-runtime'),
        expect.any(Object),
      );
    });
  });
  ...
  describe('generateAutomation', () => {
    it('sends prompt and optional device/scene IDs', async () => {
      ...
      const client = createApiClient(baseUrl, token);
      await client.generateAutomation('Turn on the lights', ['device-1'], ['scene-1']);

      const [url, options] = mockFetch.mock.calls[0]!;
      expect(url).toContain('/automations/generate');
      expect(options.method).toBe('POST');
    ...
    it('omits empty device and scene arrays', async () => {
      ...
      const client = createApiClient(baseUrl, token);
      await client.generateAutomation('Turn on the lights', [], []);
    ...
    it('uses extended timeout for generation', async () => {
      ...
      const client = createApiClient(baseUrl, token);
      await client.generateAutomation('A complex prompt');
