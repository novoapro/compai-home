...
struct LogViewerView: View {
    ...
    var automationStorageService: AutomationStorageService?
                ...
                if !viewModel.hasLogs {
                    Section {
                        EmptyStateView(
                            ...
                            title: "No logs yet",
                            message: "Logs will appear here when devices change state or automations execute."
                        )
                    ...
                    ForEach(viewModel.groupedLogs, id: \.date) { group in
                        Section(header:
                            ...
                            ForEach(group.logs) { log in
                                if let wfLog = log.automationExecution {
                                    NavigationLink {
                                        AutomationExecutionLogDetailView(logId: wfLog.id, viewModel: viewModel, onCancel: onCancelExecution)
                                    } label: {
                                        AutomationExecutionLogRow(log: wfLog)
                                    }
                                    .listRowBackground(Theme.contentBackground)
                                } else if log.category == .backupRestore,
                                          let registry = registryService,
                                          let hkManager = homeKitManager,
                                          let wfStorage = automationStorageService {
                                    NavigationLink {
                                        OrphanedDevicesView(registryService: registry, homeKitManager: hkManager, automationStorageService: wfStorage)
                                    } label: {
                                        LogRow(log: log)
                                    }
                                    .listRowBackground(Theme.contentBackground)
