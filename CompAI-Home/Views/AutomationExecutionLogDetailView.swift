...
struct AutomationExecutionLogDetailView: View {
    private let logId: UUID
    private let staticLog: AutomationExecutionLog?
    ...
    /// Static snapshot initializer — used from AutomationDetailView.
    init(log: AutomationExecutionLog, onCancel: ((UUID) -> Void)? = nil) {
        self.logId = log.id
        self.staticLog = log
        self.onCancel = onCancel
    ...
    private var log: AutomationExecutionLog? {
        viewModel.viewModel?.automationExecutionLog(id: logId) ?? staticLog
    }
    ...
    @ViewBuilder
    private func logContent(_ log: AutomationExecutionLog) -> some View {
        List {
            // Header
            Section {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    if log.status == .running, onCancel != nil {
                        ...
                        .padding(.top, 4)
                        .confirmationDialog(
                            "Kill this automation execution?",
                            isPresented: $showingKillConfirmation,
                            titleVisibility: .visible
                        ) {
                            Button("Kill Automation", role: .destructive) {
                                onCancel?(logId)
                            }
                        } message: {
                            Text("This will immediately cancel the running automation. Any in-progress actions may be left incomplete.")
                        }
                    }
                }
            }
    ...
    private func executionDuration(_ log: AutomationExecutionLog) -> String? {
...
#Preview("Success") {
    NavigationStack {
        AutomationExecutionLogDetailView(log: PreviewData.sampleAutomationLogs[1])
    }
}

#Preview("Running") {
    NavigationStack {
        AutomationExecutionLogDetailView(log: PreviewData.sampleAutomationLogs[0])
    }
}

#Preview("Failed") {
    NavigationStack {
        AutomationExecutionLogDetailView(log: PreviewData.sampleAutomationLogs[2])
    }
}
