...
struct AutomationBuilderView: View {
    ...
    @State private var generatedAutomation: Automation?
    ...
    private var currentPhase: BuilderPhase {
        generatedAutomation != nil ? .review : .describe
    }
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("AI Automation Builder")
            ...
            .sheet(isPresented: $showingDiagnostics) {
                AIInteractionLogView(loggingService: aiAutomationService.loggingService)
            }
        }
    }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    Text("Describe what you want to happen in plain English. The AI will generate a automation based on your available devices.")
                        .font(.subheadline)
                ...
                Button {
                    ...
                    HStack {
                        ...
                        Text(isGenerating ? "Generating..." : "Generate Automation")
                            .fontWeight(.semibold)
                    }
    ...
    private var reviewPhase: some View {
            ...
            if let automation = generatedAutomation {
                ...
                Section {
                    ForEach(Array(automation.triggers.enumerated()), id: \.offset) { _, trigger in
                        AutomationBuilderTriggerRow(trigger: trigger, devices: devices)
                    }
                } header: {
                ...
                if let conditions = automation.conditions, !conditions.isEmpty {
                    Section {
                        ForEach(Array(conditions.enumerated()), id: \.offset) { _, condition in
                            AutomationBuilderConditionRow(condition: condition, devices: devices, scenes: scenes)
                        }
                    } header: {
                ...
                Section {
                    ForEach(Array(automation.blocks.enumerated()), id: \.offset) { index, block in
                        AutomationBuilderBlockRow(block: block, index: index, depth: 0, devices: devices, scenes: scenes)
                    }
                } header: {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Button {
                            ...
                            HStack {
                                ...
                                Text(isGenerating ? "Refining..." : "Refine Automation")
                            }
                        }
                ...
                Section {
                    Button {
                        ...
                        Label("Save Automation", systemImage: "checkmark.circle")
                            .fontWeight(.semibold)
                    }
        ...
        .alert("Save Automation?", isPresented: $showingSaveConfirmation) {
            ...
            Text("This will save \"\(generatedAutomation?.name ?? "")\" to your automations. You can edit it later.")
        }
    }

    // MARK: - Actions

    private func generateAutomation() {
        errorMessage = nil
        isGenerating = true
        ...
        Task {
            do {
                ...
                let automation = try await aiAutomationService.generateAutomation(from: description, deviceIds: dIds, sceneIds: sIds)
                await MainActor.run {
                    self.generatedAutomation = automation
    ...
    private func refineAutomation() {
        guard let current = generatedAutomation else { return }
        errorMessage = nil
        isGenerating = true

        Task {
            do {
                let refined = try await aiAutomationService.refineAutomation(current, feedback: refinementFeedback)
                await MainActor.run {
                    self.generatedAutomation = refined
...
private struct AutomationBuilderTriggerRow: View {
...
private struct AutomationBuilderConditionRow: View {
        ...
        case .and(let conditions):
            VStack(alignment: .leading, spacing: 4) {
                ...
                ForEach(Array(conditions.enumerated()), id: \.offset) { _, child in
                    AutomationBuilderConditionRow(condition: child, devices: devices, scenes: scenes)
                        .padding(.leading, 12)
                }
            }
        ...
        case .not(let inner):
            VStack(alignment: .leading, spacing: 4) {
                Text("NOT")
                    ...
                    .fontWeight(.bold)
                    .foregroundColor(.red)
                AutomationBuilderConditionRow(condition: inner, devices: devices, scenes: scenes)
                    .padding(.leading, 12)
            }
...
private struct AutomationBuilderBlockRow: View {
...
private struct BuilderFlowControlBlockRow: View {
        ...
        VStack(alignment: .leading, spacing: 4) {
            ...
            if let nestedBlocks = flowControlNestedBlocks {
                ForEach(Array(nestedBlocks.enumerated()), id: \.offset) { i, nested in
                    AutomationBuilderBlockRow(block: nested, index: i, depth: depth + 1, devices: devices, scenes: scenes)
                }
            }
        }
        .padding(.vertical, 2)
    ...
    private var flowControlTitle: String {
        ...
        case .executeAutomation(let b): return b.name ?? "Execute Automation (\(b.executionMode.rawValue))"
        }
    }
...
#Preview {
    AutomationBuilderView(
        aiAutomationService: PreviewData.previewAIAutomationService,
