...
struct CloudBackupListView: View {
    ...
    var body: some View {
        ...
        .alert("Restore Backup", isPresented: $showingRestoreConfirmation) {
            ...
            if let backup = selectedBackup {
                Text("This will replace all current settings, automations, and device configurations with the backup from \(backup.deviceName) created on \(backup.createdAt.formatted(date: .abbreviated, time: .shortened)).")
            }
        }
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("Back up your settings and automations to iCloud from the Settings page.")
                .font(.footnote)
                .foregroundColor(Theme.Text.tertiary)
