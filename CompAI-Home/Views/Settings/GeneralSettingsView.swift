...
struct GeneralSettingsView: View {
    ...
    @State private var showingClearAutomationsConfirmation = false
    ...
    var body: some View {
        Form {
                    ...
                    HStack(alignment: .top, spacing: 10) {
                        VStack(spacing: 10) {
                            ...
                            LogCategoryCard(
                                ...
                                title: "Automation+",
                                color: .green,
                                isEnabled: $viewModel.automationLoggingEnabled,
                                mode: .executedPicker($viewModel.logSkippedAutomations)
                            )
                        }
                        VStack(spacing: 10) {
                            LogCategoryCard(
            ...
            Section {
                ...
                Button("Clear All Automations", role: .destructive) {
                    showingClearAutomationsConfirmation = true
                }
                ...
                Text("Reset Device Settings disables all characteristics (enabled and observed off). Clear All Automations removes every automation. Clear Device Registry wipes the entire registry and re-imports all devices from HomeKit with new stable IDs.")
            }
        }
        .formStyle(.grouped)
        ...
        .alert("Clear All Automations?", isPresented: $showingClearAutomationsConfirmation) {
            Button("Cancel", role: .cancel) { }
            Button("Clear", role: .destructive) {
                Task {
                    await viewModel.automationStorageService.deleteAllAutomations()
                }
            }
        } message: {
            Text("This will permanently delete all automations. This action cannot be undone.")
        }
        .alert("Clear Device Registry?", isPresented: $showingClearRegistryConfirmation) {
            ...
            Text("This will wipe the entire device registry and re-import all devices from HomeKit with new stable IDs. Any existing automation references to devices will become orphaned.")
        }
    }
}
