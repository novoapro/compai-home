...
struct AIAssistantSettingsView: View {
    ...
    var body: some View {
        Form {
            Section {
                Toggle("Enable AI Automation Builder", isOn: $viewModel.aiEnabled)
                ...
                Text("Configure an LLM provider to create automations from natural language descriptions.")
            }
            ...
            Section {
                DisclosureGroup("System Prompt", isExpanded: $showingSystemPrompt) {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        HStack {
                            if viewModel.aiSystemPrompt == AIAutomationService.defaultSystemPrompt {
                ...
                Text("The system prompt defines how the AI generates automations. Device and scene context is automatically appended to each request.")
            }
            .disabled(!viewModel.aiEnabled)
            .opacity(viewModel.aiEnabled ? 1 : 0.5)
            ...
            Section {
                ...
                Button("Clear AI History", role: .destructive) {
                    Task { await aiAutomationService.loggingService.clearLogs(forCategories: [.aiInteraction, .aiInteractionError]) }
                }
            } header: {
                Label("Diagnostics", systemImage: "wrench.and.screwdriver")
            } footer: {
                Text("View prompts, responses, and errors from AI automation generation attempts.")
            }
        }
        .formStyle(.grouped)
...
#Preview {
    NavigationStack {
        AIAssistantSettingsView(
            ...
            aiAutomationService: PreviewData.settingsViewModel.aiAutomationService
        )
    }
}
