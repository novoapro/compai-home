...
enum LogCategoryFilter: String, CaseIterable, Identifiable {
    ...
    case automationExecution = "Automation"
    ...
    var logCategories: [LogCategory]? {
        ...
        case .automationExecution: return [.automationExecution, .automationError]
    ...
    var icon: String {
        ...
        case .automationExecution: return "bolt.circle.fill"
...
class LogViewModel: ObservableObject {
    ...
    /// Returns the latest version of a automation execution log by ID, for live detail views.
    func automationExecutionLog(id: UUID) -> AutomationExecutionLog? {
        rawLogs.first(where: { $0.id == id })?.automationExecution
    }
    ...
    private static func filterLogs(
        ...
        return result.filter { log in
            ...
            // Automation-specific search
            if let wf = log.automationExecution {
                if wf.automationName.localizedCaseInsensitiveContains(lowerQuery) { return true }
