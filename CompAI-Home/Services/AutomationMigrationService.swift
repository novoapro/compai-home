...
/// Migrates orphaned device/service/scene UUIDs in automations after a HomeKit backup restore.
///
...
enum AutomationMigrationService {

    /// A reference in a automation that could not be resolved to any known device or scene.
    struct OrphanedReference {
        let referenceId: String
        let referenceName: String?
        let roomName: String?
        /// Where in the automation the orphan was found (e.g. "trigger", "condition", "block (runScene)").
        let location: String
        /// Whether this is a scene reference (vs. device reference).
        let isScene: Bool
    }

    /// Result of a migration attempt on a single automation.
    ...
    /// Migrate all device/service/scene references in a automation.
    /// Returns the (possibly updated) automation and a summary of changes.
    static func migrate(_ automation: Automation, using devices: [DeviceModel], scenes: [SceneModel] = [], registry: DeviceRegistryService? = nil) -> MigrationResult {
        ...
        let allDeviceRefs = collectDeviceReferences(from: automation)
        ...
        let allSceneRefs = collectSceneReferences(from: automation)
        ...
        if idMap.isEmpty && sceneIdMap.isEmpty {
            return MigrationResult(automation: automation, remappedDevices: 0, remappedServices: 0, remappedScenes: 0, orphanedReferences: orphanedRefs)
        }

        // Apply the remapping by encoding → modifying JSON → decoding
        let migratedAutomation = applyRemapping(to: automation, deviceIdMap: idMap, serviceIdMap: serviceIdMap, sceneIdMap: sceneIdMap)

        return MigrationResult(
            automation: migratedAutomation ?? automation,
    ...
    /// A lightweight reference to a device used somewhere in a automation.
    struct DeviceRef: Hashable {
        ...
        /// Where in the automation this reference appears (e.g. "trigger", "condition", "block").
        let location: String
    ...
    static func collectDeviceReferences(from automation: Automation) -> Set<DeviceRef> {
    ...
    private static func collectConditionRefs(_ condition: AutomationCondition, into refs: inout Set<DeviceRef>) {
        switch condition {
    ...
    private static func collectBlockRefs(_ block: AutomationBlock, into refs: inout Set<DeviceRef>) {
        switch block {
    ...
    /// A lightweight reference to a scene used somewhere in a automation.
    ...
    static func collectSceneReferences(from automation: Automation) -> Set<SceneRef> {
    ...
    private static func collectSceneConditionRefs(_ condition: AutomationCondition, into refs: inout Set<SceneRef>) {
        switch condition {
    ...
    private static func collectSceneBlockRefs(_ block: AutomationBlock, into refs: inout Set<SceneRef>) {
        switch block {
        case let .action(action, _):
            switch action {
            case let .runScene(a):
    ...
    static func applyRemapping(to automation: Automation, deviceIdMap: [String: String], serviceIdMap: [String: [String: String]], sceneIdMap: [String: String] = [:]) -> Automation? {
        let encoder = JSONEncoder()
        encoder.dateEncodingStrategy = .iso8601

        guard let jsonData = try? encoder.encode(automation),
        ...
        guard let updatedData = jsonString.data(using: .utf8),
              let updatedAutomation = try? decoder.decode(Automation.self, from: updatedData) else {
            return nil
        }
...
extension AutomationMigrationService {
    /// One-time migration: replaces all HomeKit UUIDs in automations with stable registry IDs.
    /// Uses the registry's nonisolated sync lookups so this can be called from any context.
    ///
    /// For each deviceId/serviceId/sceneId in a automation, checks if it's a known HomeKit UUID
    /// and replaces it with the corresponding stable registry ID.
    static func migrateToStableIds(_ automations: [Automation], registry: DeviceRegistryService) -> (automations: [Automation], migratedCount: Int) {
        var deviceIdMap: [String: String] = [:]
        var serviceIdMap: [String: String] = [:]
        var sceneIdMap: [String: String] = [:]

        // Collect all unique IDs from all automations and build remapping tables
        for automation in automations {
            for ref in collectDeviceReferences(from: automation) {
            ...
            for ref in collectSceneReferences(from: automation) {
        ...
        AppLogger.automation.info("Registry migration: remapped \(deviceIdMap.count) device(s), \(serviceIdMap.count) service(s), \(sceneIdMap.count) scene(s)")
...
extension AutomationMigrationService {
    /// Result of a batch migration across multiple automations.
    struct BatchMigrationResult {
        let automations: [Automation]
        let totalRemappedDevices: Int
        let totalRemappedScenes: Int
        let orphanedReferences: [String: [OrphanedReference]]  // automationName → orphans
    }

    /// Migrate multiple automations at once. Returns updated automations, remapping counts, and orphan details.
    static func migrateAll(_ automations: [Automation], using devices: [DeviceModel], scenes: [SceneModel] = [], registry: DeviceRegistryService? = nil) -> BatchMigrationResult {
        var result: [Automation] = []
        ...
        for automation in automations {
            let migration = migrate(automation, using: devices, scenes: scenes, registry: registry)
            result.append(migration.automation)
            ...
            if !migration.orphanedReferences.isEmpty {
                allOrphans[automation.name] = migration.orphanedReferences
            }
        }

        let totalRemapped = totalRemappedDevices + totalRemappedScenes
        if totalRemapped > 0 {
            AppLogger.automation.info("Automation migration: remapped \(totalRemappedDevices) device(s), \(totalRemappedScenes) scene(s) across \(automations.count) automation(s)")
        }
        if !allOrphans.isEmpty {
            let totalOrphans = allOrphans.values.map(\.count).reduce(0, +)
            AppLogger.automation.warning("Automation migration: \(totalOrphans) orphaned reference(s) in \(allOrphans.count) automation(s) could not be resolved")
        }
...
extension AutomationMigrationService {
    ...
    /// Result of validating a single automation reference against the registry.
    ...
    /// Result of validating and repairing all automation references.
    ...
    /// Validates ALL automation references (deviceId, serviceId, characteristicType)
    ...
    static func validateAndRepairReferences(
        ...
        for automation in automations {
            let deviceRefs = collectDeviceReferences(from: automation)
        ...
        let charTypes = collectAllCharacteristicTypes(from: automations)
        ...
        if serviceRemapTable.isEmpty && charTypeRemapTable.isEmpty {
            return ValidationResult(
                updatedAutomations: automations,
                autoFixed: autoFixed,
    ...
    /// Collects ALL characteristicType strings from all automation triggers, conditions, and blocks.
    static func collectAllCharacteristicTypes(from automations: [Automation]) -> Set<String> {
    ...
    private static func collectTriggerCharTypes(_ trigger: AutomationTrigger, into types: inout Set<String>) {
        switch trigger {
    ...
    private static func collectConditionCharTypes(_ condition: AutomationCondition, into types: inout Set<String>) {
        switch condition {
    ...
    private static func collectBlockCharTypes(_ block: AutomationBlock, into types: inout Set<String>) {
        switch block {
...
// TODO: Remove this extension once all existing automations have been migrated.
...
extension AutomationMigrationService {
    ...
    static func migrateCharacteristicIds(_ automations: [Automation], registry: DeviceRegistryService) -> (automations: [Automation], migratedCount: Int) {
        ...
        for automation in automations {
            ...
            if let updatedData = jsonString.data(using: .utf8),
               let updatedAutomation = try? decoder.decode(Automation.self, from: updatedData) {
                result.append(updatedAutomation)
            } else {
                result.append(automation)
            }
        }
    ...
    private static func collectTriggerCharRefs(_ trigger: AutomationTrigger, into refs: inout Set<CharRef>) {
        switch trigger {
        case .deviceStateChange(let t):
    ...
    private static func collectConditionCharRefs(_ condition: AutomationCondition, into refs: inout Set<CharRef>) {
        switch condition {
        case .deviceState(let c):
    ...
    private static func collectBlockCharRefs(_ block: AutomationBlock, into refs: inout Set<CharRef>) {
        switch block {
        case .action(let action, _):
            switch action {
