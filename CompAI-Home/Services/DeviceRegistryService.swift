...
/// Automations reference stable IDs (which never change). The registry maps those to
/// HomeKit UUIDs (which can differ per device or after backup restore).
/// If HomeKit UUIDs change, update the registry once — all automations automatically work.
actor DeviceRegistryService {
    ...
    /// Returns a remapping table so callers can update automation references from the removed stableId to the resolved one.
    ...
    /// consumer (REST API, MCP tools, automation editor, AI service).
    ...
    /// An orphaned service reference found in a automation.
    struct UnresolvedServiceRef: Identifiable {
        var id: String { "\(deviceStableId)-\(serviceId)-\(automationName)-\(location)" }
    ...
    /// Returns automation service references that point to a serviceId not present in the registry.
    /// Only checks resolved devices (unresolved devices are handled by existing orphan detection).
    func unresolvedServiceReferences(in automations: [Automation]) -> [UnresolvedServiceRef] {
        var results: [UnresolvedServiceRef] = []
        var seen = Set<String>()

        for automation in automations {
            let refs = AutomationMigrationService.collectDeviceReferences(from: automation)
            for ref in refs {
    ...
    /// Preserves the stable ID so all automation references remain valid.
    ...
    /// The target service adopts the orphaned stable ID so automation references continue to resolve.
    func remapService(
    ...
    // MARK: - Automation Reconciliation

    /// Reconciles foreign stable IDs in automations against the local registry.
    ///
    /// When automations arrive from another device (CloudKit sync) or from a backup,
    ...
    func reconcileAutomationReferences(
        ...
        // Collect all device and scene references from automations
        for automation in automations {
        ...
        if created > 0 {
            rebuildReverseLookups()
            debouncedSave()
            AppLogger.registry.info("Reconciliation: created \(created) registry entries from automation references")
        }
    ...
    // MARK: - Automation Dependency Tracking

    /// Finds all automations that reference a given device stable ID.
    /// Returns tuples of (automationName, locations) where locations describe where in the automation the reference appears.
    nonisolated func findAutomationsReferencing(
        deviceStableId: String,
        in automations: [Automation]
    ) -> [(automationName: String, locations: [String])] {
        var results: [(automationName: String, locations: [String])] = []
        for automation in automations {
            let refs = AutomationMigrationService.collectDeviceReferences(from: automation)
            let matchingLocations = refs.filter { $0.deviceId == deviceStableId }.map(\.location)
            if !matchingLocations.isEmpty {
                results.append((automationName: automation.name, locations: matchingLocations))
            }
        }
        return results
    }

    /// Finds all automations that reference a given scene stable ID.
    nonisolated func findAutomationsReferencing(
        ...
        var results: [(automationName: String, locations: [String])] = []
        for automation in automations {
            let refs = AutomationMigrationService.collectSceneReferences(from: automation)
