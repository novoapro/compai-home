...
/// Manages timer-based schedule triggers for automations.
/// Registers active schedules and fires them via AutomationEngine.
actor ScheduleTriggerManager {
    private var scheduledTasks: [UUID: [Task<Void, Never>]] = [:]
    private weak var engine: AutomationEngine?
    private var storage: StorageService?

    func setEngine(_ engine: AutomationEngine) {
        self.engine = engine
    }
    ...
    /// Reload all schedules from the given automations.
    func reloadSchedules(automations: [Automation]) {
        cancelAll()
        for automation in automations where automation.isEnabled {
            registerSchedules(for: automation)
        }
    }

    /// Register schedule triggers for a single automation.
    func registerSchedules(for automation: Automation) {
        ...
        if !tasks.isEmpty {
            scheduledTasks[automation.id] = tasks
        }
    }

    /// Cancel schedules for a specific automation.
    func cancelSchedules(for automationId: UUID) {
        scheduledTasks[automationId]?.forEach { $0.cancel() }
        scheduledTasks.removeValue(forKey: automationId)
    }
    ...
    private func createScheduleTask(automationId: UUID, trigger: ScheduleTrigger) -> Task<Void, Never> {
        let policy = trigger.retriggerPolicy
        return Task { [weak self] in
            switch trigger.scheduleType {
            case .once(let date):
                await self?.scheduleOnce(automationId: automationId, trigger: trigger, date: date, policy: policy)
            case .daily(let time):
                await self?.scheduleRepeating(automationId: automationId, trigger: trigger, policy: policy) {
            ...
            case .interval(let seconds):
                await self?.scheduleInterval(automationId: automationId, trigger: trigger, seconds: seconds, policy: policy)
            }
        }
    }

    private func scheduleOnce(automationId: UUID, trigger: ScheduleTrigger, date: Date, policy: ConcurrentExecutionPolicy?) async {
        let delay = date.timeIntervalSinceNow
        guard delay > 0 else { return }

        do {
            try await Task.sleep(nanoseconds: UInt64(delay * 1_000_000_000))
            await fireTrigger(automationId: automationId, trigger: trigger, policy: policy)
        } catch {
            // Task cancelled
        }
    }

    private func scheduleRepeating(automationId: UUID, trigger: ScheduleTrigger, policy: ConcurrentExecutionPolicy?, nextDate: @Sendable () -> Date?) async {
    ...
    private func scheduleInterval(automationId: UUID, trigger: ScheduleTrigger, seconds: TimeInterval, policy: ConcurrentExecutionPolicy?) async {
        while !Task.isCancelled {
            do {
    ...
    private func fireTrigger(automationId: UUID, trigger: ScheduleTrigger, policy: ConcurrentExecutionPolicy?) async {
        guard let engine else { return }
        guard storage?.readAutomationsEnabled() == true else { return }
        ...
        _ = await engine.scheduleTrigger(id: automationId, triggerEvent: event, policy: policy, triggerConditions: trigger.conditions)
    }
    ...
    private func createSunEventTask(automationId: UUID, trigger: SunEventTrigger) -> Task<Void, Never> {
        let policy = trigger.retriggerPolicy
        return Task { [weak self] in
            await self?.scheduleSunEventRepeating(automationId: automationId, trigger: trigger, policy: policy)
        }
    }

    private func scheduleSunEventRepeating(automationId: UUID, trigger: SunEventTrigger, policy: ConcurrentExecutionPolicy?) async {
        while !Task.isCancelled {
            let latitude = storage?.readSunEventLatitude() ?? 0
            let longitude = storage?.readSunEventLongitude() ?? 0

            guard latitude != 0 || longitude != 0 else {
                AppLogger.automation.warning("Sun event trigger: location not configured, retrying in 5 minutes")
            ...
            guard let fireDate = nextFireDate else {
                AppLogger.automation.warning("Sun event trigger: could not compute next \(trigger.event.displayName) time (polar region?)")
                try? await Task.sleep(nanoseconds: 3600_000_000_000)
                continue
            }
            ...
            AppLogger.automation.info("Sun event trigger: next \(trigger.event.displayName) at \(formatter.string(from: fireDate)) (in \(Int(delay))s)")

            do {
                try await Task.sleep(nanoseconds: UInt64(delay * 1_000_000_000))
                guard !Task.isCancelled else { return }
                await fireSunEventTrigger(automationId: automationId, trigger: trigger, policy: policy)
                // Wait 61 seconds to avoid re-triggering in the same minute
                try await Task.sleep(nanoseconds: 61_000_000_000)
            } catch {
                return // Cancelled
    ...
    private func fireSunEventTrigger(automationId: UUID, trigger: SunEventTrigger, policy: ConcurrentExecutionPolicy?) async {
        guard let engine else { return }
        guard storage?.readAutomationsEnabled() == true else { return }
